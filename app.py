...
# --- Load model safely (works on both Windows and Render) ---
...
with col1:
    ...
    Make_name = st.selectbox("Make", options=list(car_mapping.keys()))
    Make = car_mapping[Make_name]
...
with col2:
    ...
    Width = st.number_input("Width", value=64.1)

# --- Combine inputs ---
X_input = np.array([[
    Make, Fuel_type, aspiration, Doors, Body_style,
    Drive_wheels, Engine_location, Engine_type, num_of_cylinders, Fuel_system,
    Wheel_base, Height, Engine_size, Bore, Stroke, Compression_ratio, Peak_rpm,
    City_mpg, Width, symboling, Normalized_loss
]])
